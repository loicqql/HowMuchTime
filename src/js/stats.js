...
function displayMessage(e) {
    document.querySelector('.message p').textContent = e;
...
function updateDark() {
    getAllData((data) => {
        ...
        data[0]['is_dark'] ? document.querySelector(".stats").classList.add('dark') : document.querySelector(".stats").classList.remove('dark');
        browser.storage.local.set({
            [KEY]: data
        })
    });
...
function displayTotalTime() {
    getTotalTime((result) => {
        document.querySelector("header .title p").textContent = result;
    });
}